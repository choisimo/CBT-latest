...
public class AuthenticationFilter extends UsernamePasswordAuthenticationFilter implements PluggableFilter {
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            // Read the request body from the wrapped request
            Map<String, String> credentials = objectMapper.readValue(wrappedRequest.getInputStream(), Map.class);
            String email = credentials.get("email");
            String loginId = credentials.get("loginId");
            String username = credentials.get("username"); // for backward compatibility / alternative
            String password = credentials.get("password");

            String identifier = null;

            log.info("로그인 요청 payload 상세: email='{}', loginId='{}', username='{}', password길이={}", 
                    email, loginId, username, (password != null ? password.length() : "null"));

            // Try to use email first
            if (isValidIdentifier(email)) {
                identifier = email;
                log.info("이메일을 식별자로 사용: {}", identifier);
            }
            // If email is not valid, try loginId
            else if (isValidIdentifier(loginId)) {
                identifier = loginId;
                log.info("로그인ID를 식별자로 사용: {}", identifier);
            }
            // If loginId is not valid, try username (as a fallback)
            else if (isValidIdentifier(username)) {
                identifier = username;
                log.info("사용자명을 식별자로 사용: {}", identifier);
            }

            log.info("최종 추출된 identifier: {}", identifier);

            if (identifier == null || password == null || password.isBlank()) {
                log.error("로그인 요청에 유효한 식별자 또는 비밀번호가 누락되었습니다. 전체 페이로드: {}", credentials);
                log.error("식별자 검증 결과: email valid={}, loginId valid={}, username valid={}", 
                        isValidIdentifier(email), isValidIdentifier(loginId), isValidIdentifier(username));
                // Using INVALID_REQUEST as per original behavior for missing credentials
            ...
            // Using AUTHENTICATION_FAILED as per original behavior for IO errors during parsing
