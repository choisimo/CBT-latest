...
export default function SignupScreen({ navigation }: Props) {
  ...
  const handleSignup = async () => {
    try {
      ...
      if (!isJson) {
        console.error('서버가 JSON이 아닌 응답을 반환했습니다. Content-Type:', contentType);
        const responseText = await res.text();
        console.error('응답 내용:', responseText.substring(0, 500)); // 처음 500자만 로깅
        Alert.alert('오류', '서버 연결에 문제가 있습니다. 잠시 후 다시 시도해주세요.');
        return;
      }
      ...
      if (error.message && error.message.includes('JSON Parse error')) {
        Alert.alert('오류', '서버 연결에 문제가 있습니다. 서버가 실행 중인지 확인해주세요.');
