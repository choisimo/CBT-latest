...
export default function SignupScreen({ navigation }: Props) {
  ...
  const handleSendEmailCode = async () => {
    ...
    // safeApiCall을 사용하여 이메일 인증번호 요청
    const result = await safeApiCall(
      `${BASIC_URL}/api/public/emailCode`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    if (result.success) {
      ...
      console.log('🔴 Email code send error:', result.error);
  ...
  const handleConfirmEmailCode = async () => {
    ...
    if (result.success) {
      ...
      Alert.alert('오류', result.error || '인증 확인 중 오류가 발생했습니다.');
    }
  };
