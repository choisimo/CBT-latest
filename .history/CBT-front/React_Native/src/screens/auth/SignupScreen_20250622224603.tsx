...
export default function SignupScreen({ navigation }: Props) {
  ...
  const checkDuplication = async (
    ...
    const result = await safeApiCall(
      `${BASIC_URL}/api/public/check/${endpoint}/IsDuplicate`,
      {
        method: 'POST',
    ...
    if (result.success) {
      const res_json = result.data as Res;
      ...
      console.error(`${fieldName} 중복 확인 실패:`, result.error);
      return {
        isDuplicate: true, // 에러 시 안전하게 중복으로 처리
        isAvailable: false,
  ...
  const handleSendEmailCode = async () => {
    ...
    const result = await safeApiCall(`${BASIC_URL}/api/public/emailCode`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (result.success) {
      const { data } = result;
      Alert.alert(
        data.status === 'success' ? '확인' : '오류',
        data.message
      );
    } else {
      Alert.alert('오류', result.error || '인증번호 발송에 실패했습니다.');
    }
  };
  ...
  const handleSignup = async () => {
    console.log('회원가입 시도:', { loginId, email, nickname });
